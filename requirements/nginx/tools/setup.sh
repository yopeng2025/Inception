...
# This path must match 'ssl_certificate' in your nginx.conf
...
if [ ! -f "/etc/nginx/ssl/inception.crt" ]; then
    ...
    # -subj: Pre-fills certificate identity information (Country, State, Locality, Organization, Common Name)
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
        -keyout /etc/nginx/ssl/inception.key \
        -out /etc/nginx/ssl/inception.crt \
        -subj "/C=FR/ST=Paris/L=Paris/O=42/OU=42/CN=$DOMAIN_NAME/UID=$USER"
